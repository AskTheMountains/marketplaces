...
# Функция создания диапазона дат по дням
def generate_date_range(date_start, date_end):
    date_range = pd.DataFrame({'date': pd.date_range(date_start, date_end, freq='d')})
    date_range['date'] = date_range['date'].dt.date

    return date_range

# Функция создания диапазона дат по дням (новая)
def generate_date_range_new():
...
if __name__ == '__main__':
    ...
    date_start, date_end = generate_date_range_new()
